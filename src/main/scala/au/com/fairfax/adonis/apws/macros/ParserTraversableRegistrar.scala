...
object ParserTraversableRegistrar {
  ...
  def materialiseDirect(c: Context)(tpe: c.Type): c.Expr[ParserTraversableRegistrar[_]] = {
    import c.universe._
    val tpeStr = toMapKey(tpe.toString)

    println(s"ParserTraversableRegistrar.materialize(), tpe = $tpe, tpeStr = $tpeStr")
    ...
    c.Expr[ParserTraversableRegistrar[_]](result)
  }
    ...
    q"""
      ...
      implicit object GenTraversableRegistrar extends ParserTraversableRegistrar[${tpe}] {
        val traversableRegister: Unit = {
          JsonRegistry.addParser(($keyBeAdded, $parser))
  ...
  private def registerToRegistryQuote(c: Context)(registerTpe: c.universe.Type): c.universe.Tree = {
    ...
    if (!(ParserRegistrarMacroTracker contains registerTpe.toString)) {
